...
const Dash = () => {
  ...
  const users: EntityId[] = useSelector((state: RootState) =>
    selectUserIds(state),
  );
  const notes: EntityId[] = useSelector((state: RootState) =>
    selectNoteIds(state),
  );
  const forAdmin = (
    <section>
      ...
      <h3>
        <Link to={"/dash/users"}>Manage Users</Link>
      </h3>
    </section>
  );
  ...
  return (
    <section>
      ...
      <h3>
        <Link to={"/dash/notes"}>View My Notes</Link>
      </h3>
      <h3>
