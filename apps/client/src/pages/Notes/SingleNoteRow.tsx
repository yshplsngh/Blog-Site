...
import { resNotesArrayType, errTypo } from "@repo/types";
...
const SingleNoteRow = React.memo(({ noteId, email }: noteIdType) => {
  ...
  const [deleteNote, { isError, error, isSuccess, isLoading }] =
  ...
  if (note as resNotesArrayType) {
    ...
    const handleEdit = () => {
      navigate(`/dash/notes/edit/${noteId}`, {
        state: { email: email, prevUrl: location.pathname },
      });
    };
    const handleView = () => {
