...
const envVarsSchema = z.object({
  ACCESS_TOKEN_SECRET: z
    .string()
    .min(1)
    .refine((data: string): boolean => data.trim() !== "", {
      message: "Provide access token secret",
    }),
  REFRESH_TOKEN_SECRET: z
    .string()
    .min(1)
    .refine((data: string): boolean => data.trim() !== "", {
      message: "Provide refresh token secret",
    }),
  GEMINI_AI: z
    .string()
    .min(1)
    .refine((data: string): boolean => data.trim() !== "", {
      message: "Provide gemini ai key",
    }),
  MONGO_URI: z
    .string()
    .min(1)
    .refine((data: string): boolean => data.trim() !== "", {
      message: "Provide mongo uri",
    }),
});
