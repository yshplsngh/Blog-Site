...
const JWTverify = (
  ...
  if (!headerData) {
    ...
    return res.status(401).send({
      success: false,
      message: "Unauthorised/access token not found in header|jwtVerify",
    });
  }
  ...
  if (!data.startsWith("Bearer")) {
    ...
    return res.status(401).send({
      success: false,
      message: "Unauthorized/access token did not start with Bearer|jwtVerify",
    });
  }
  const orgToken: string | undefined = data.split(" ")[1];
      ...
      async (err: any, decoded: any) => {
        if (err) {
          ...
          return res.status(403).send({
            success: false,
            message:
              "UnAuthorized/ expired or modified jwt in header|jwtVerify",
          });
        }
